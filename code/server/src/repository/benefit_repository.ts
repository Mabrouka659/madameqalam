import type Benefit from "../model/benefit.js";
import MySQLService from "../service/mysql_service.js";

class BenefitRepository{
	// nom de la table SQL
	private table = "benefint";

	// récupérer tous les enregistrements
	//async crée une promesse
	//la fonction renvoie un objet unkouwn lorsqu'une erreur est renvoyée
	public selectAll = async (): Promise<Benefit[] | unknown> => {
	...
	public selectOne = async (data: Partial<Benefit>): Promise<Benefit | unknown> => {
		// connexion au serveur MYSQL
		const connection = await new MySQLService().connect();
    ...
    `;
		...
		try {
			...
			const result = (results as Benefit[]).shift();
